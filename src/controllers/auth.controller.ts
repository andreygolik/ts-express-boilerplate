...
// @access  Private
export const updateDetails = asyncHandler(async (req: UserRequest, res: Response) => {
...
// @access  Public
export const forgotPassword = asyncHandler(async (req: UserRequest, res: Response, next: NextFunction) => {
  const user = await UserModel.findOne({ email: req.body.email });

  if (!user) {
    return next(new ErrorResponse('Unable find that email, sorry.', 404));
  }
...
// @access  Public
export const resetPassword = asyncHandler(async (req: UserRequest, res: Response, next: NextFunction) => {
